...
import com.hjq.demo.view.InviteStuView;
import com.hjq.demo.view.JoinCourseInputView;
import com.hjq.demo.view.MeFragmentView;
import com.hjq.demo.view.TeacherMessageView;

import butterknife.BindView;

public class HomeActivityForTeacher extends MyActivity
    ...
    protected void initData() {
        ...
        mPagerAdapter.addFragment(InviteStuView.newInstance());
        mPagerAdapter.addFragment(TestFragmentB.newInstance());
        mPagerAdapter.addFragment(TeacherMessageView.newInstance());
        mPagerAdapter.addFragment(MeFragmentView.newInstance());
    ...
    protected void onResume() {
        ...
        if(loginController != null) {
            if (loginController.getUser().getUsertype() == User.TEACHER) {
                List<MyFragment> list = mPagerAdapter.getAllFragment();
                list.remove(0);
                list.add(0, InviteStuView.newInstance());
                mViewPager.setAdapter(mPagerAdapter);
            }
        }
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.home_course:
                mPagerAdapter.setCurrentItem(InviteStuView.class);
                return true;
            case R.id.home_assignment:
                mPagerAdapter.setCurrentItem(TestFragmentB.class);
                return true;
            case R.id.home_message:
                mPagerAdapter.setCurrentItem(TeacherMessageView.class);
                return true;
            case R.id.home_me:
                mPagerAdapter.setCurrentItem(MeFragmentView.class);
                return true;
            default:
                break;
        }
