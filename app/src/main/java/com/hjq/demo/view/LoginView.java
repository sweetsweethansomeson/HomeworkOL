...
public class LoginView extends MyActivity implements UmengLogin.OnLoginListener,
    ...
    @BindView(R.id.et_login_usrname)
    ...
    /** 用户类型 */
    private static int usrtype = User.STUDENT;
    /** 用户字符串ID */

    @DebugLog
    public static void start(Context context, String usrname, String password, int usrtype) {
        Intent intent = new Intent(context, LoginView.class);
        intent.putExtra(IntentKey.USRNAME, usrname);
        intent.putExtra(IntentKey.PASSWORD, password);

        LoginView.usrtype = usrtype;
    ...
    protected void initData() {
        postDelayed(() -> {
            // 在小屏幕手机上面，因为计算规则的因素会导致动画效果特别夸张，所以不在小屏幕手机上面展示这个动画效果
            if (mBlankView.getHeight() > mBodyLayout.getHeight()) {
                // 只有空白区域的高度大于登录框区域的高度才展示动画
                KeyboardWatcher.with(LoginView.this)
                        .setListener(LoginView.this);
            }
        }, 500);
        ...
        // 如果没有安装就隐藏提示
        if (mQQView.getVisibility() == View.GONE) {
            mOtherView.setVisibility(View.GONE);
        }

        // 填充传入的用户名和密码
        mUsrnameView.setText(getString(IntentKey.USRNAME));
    ...
    public void onRightClick(View v) {
        // 跳转到注册界面
        startActivityForResult(RegisterView.class, (resultCode, data) -> {
            // 如果已经注册成功，就执行登录操作
            if (resultCode == RESULT_OK && data != null) {
                mUsrnameView.setText(data.getStringExtra(IntentKey.USRNAME));
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_login_commit:
                /*if (mUsrnameView.getText().toString().length() != 11) {
                ...
                if (true) {
                    ...
                    if(usrtype == User.STUDENT)
                        startActivity(HomeActivity.class);
                    else {
                        Intent intent = new Intent();
                        intent.setClass(LoginView.this, HomeActivityForTeacher.class);
    ...
    public void onSucceed(Platform platform, UmengLogin.LoginData data) {
        ...
        mUsrnameView.setText(data.getName());
    }
    ...
    public boolean sendMessage(){
        ...
        int res = loginController.loadUser(usrname, password);
        if(res == LoginController.LOGIN_ERROR_ACCOUNT_DONT_EXIST) {
            toast("用户名不存在，请先注册");
            return false;
        }
        if(res == LoginController.LOGIN_ERROR_WRONG_PASSWORD) {
            toast("用户名与密码不符，请查验后重试");
            return false;
        }
        if(res == LoginController.LOGIN_SUCCEED){
            toast("登录成功");
            usrtype = loginController.getUser().getUsertype();
            return true;
        }
